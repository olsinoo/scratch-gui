...
class BlockTrees {
    ...
    findNodeByType (opcode) {
        const foundNodes = allBlocks.filter(value => value.opcode === opcode);
        return foundNodes;
    }

    findNodeByID (nodeID) {
        const foundNodes = allBlocks.filter(value => value.id === nodeID);
    ...
    addNode (node) {
        if (this.roots.length === 0 || node.isRoot) {
            node.childNode = this.findNodeByID(node.childID);
    ...
    updateNode (node) {
        ...
        if (node.childID !== '' && node.childNode === null) {
            node.childNode = this.findNodeByID(node.childID);
            this.updateNode(node.childNode);
        }
        if (node.parentID !== '' && node.parentNode === null) {
            node.parentNode = this.findNodeByID(node.parentID);
        }
        if (node.conditionIDs.length > 0) {
            for (const conditionID of node.conditionIDs) {
                const foundNode = this.findNodeByID(conditionID);
                this.updateNode(foundNode);
                if (foundNode !== null) {
                    node.value[node.value.indexOf(null)] = `${foundNode.opcode.substring(foundNode.opcode.indexOf('_') + 1)}(${(foundNode.value === null) ? '' : foundNode.value})`;
                }
            }
        }
        if (node.parentID !== '' &&
            node.parentNode !== null &&
            !node.parentNode.conditionIDs.includes(node.id) &&
...
class Blocks extends React.Component {
    ...
    attachVM () {
        ...
        this.props.vm.on('PROJECT_CHANGED', this.onBlockUpdate);
